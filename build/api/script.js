    ...
    route.get('/scripts/files', async (req, res, next) => {
        ...
        try {
            ...
            res.send({
                code: 200,
                data: fileList.map((x) => {
                    if (fs.lstatSync(config_1.default.scriptPath + x).isDirectory()) {
                        const childFileList = fs.readdirSync(config_1.default.scriptPath + x, 'utf-8');
                        return {
                            title: x,
                            value: x,
                            key: x,
                            ...
                            children: childFileList.map((y) => {
                                const statObj = fs.statSync(`${config_1.default.scriptPath}${x}/${y}`);
                                return {
                                    title: y,
                                    value: y,
                                    key: y,
