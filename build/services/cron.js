...
let CronService = class CronService {
    constructor(logger, scheduleService) {
        this.logger = logger;
    ...
    async disabled(ids) {
        return new Promise((resolve) => {
            this.cronDb.update({ _id: { $in: ids } }, { $set: { isDisabled: 1 } }, { multi: true, returnUpdatedDocs: true }, async (err, num, docs) => {
                for (const doc of docs) {
    ...
    async enabled(ids) {
        return new Promise((resolve) => {
            this.cronDb.update({ _id: { $in: ids } }, { $set: { isDisabled: 0 } }, { multi: true, returnUpdatedDocs: true }, async (err, num, docs) => {
                for (const doc of docs) {
                    await this.scheduleService.cancelSchedule(doc);
                    await this.scheduleService.generateSchedule(doc);
                }
